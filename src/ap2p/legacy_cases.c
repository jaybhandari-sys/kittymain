...
#include "state.h"     /* ap2p_state_t (shutdown_flag) — keepalive thread */
#include <arpa/inet.h>
...
void handle_case_8(void);            /* v2.0.7 — device info publish helper  */
void handle_case_59_set_ist(void);   /* v2.0.7 — timezone PUT to GMT+05:30   */
...
void handle_case_0() {
    /* v2.0.7-rc4 — case-0 MUST publish a heartbeat unconditionally.  IoT
     * fleet alerting decides "this camera is offline" by absence of /tx/<sn>/0
     * publishes within ~90 s.  The previous logic bailed on get_system_info()
     * failure (no SD card → statvfs("/mnt/tf") fails OR no thermal zone file
     * → fopen fails) and produced ZERO heartbeats from lab cameras, which
     * looks identical to a genuine outage.  Decoupled now:
     *   1) ALWAYS publish a minimal "Keep Alive!" so the operator sees the
     *      camera as alive even if telemetry can't be collected.
     *   2) BEST-EFFORT publish telemetry on top (temperature + SD + CPU)
     *      when available; failures are logged but don't suppress the beat. */
    printf("Case 0: Publishing 30 s heartbeat to torque/tx/<sn>/0...\n");

    /* (2) Best-effort telemetry first — fills `response` for the publish below. */
    const char* system_info = get_system_info();
    if (system_info && system_info[0]) {
        /* get_system_info already snprintf'd into result_buffer.  Copy into
         * `response` (the shared publish buffer).  Cast away const safely:
         * we strncpy, not mutating system_info. */
        strncpy(response, system_info, sizeof(response) - 1);
        response[sizeof(response) - 1] = '\0';
        printf("Publishing system info: %s\n", response);
    } else {
        /* Fallback payload — minimal valid JSON the server can parse. */
        snprintf(response, sizeof(response),
                 "{\"keepalive\":true,\"telemetry\":\"unavailable\"}");
        printf("Publishing fallback heartbeat (no telemetry): %s\n", response);
    }

    /* (1) Always publish — even if response is the fallback. */
...
int legacy_message_arrived(void *context, char *topicName, int topicLen, MQTTClient_message *message) {
    ...
    if (!safePayload) {
        ...
        /* AP2P: caller owns message + topicName lifetime, do not free here */ return 1;
    }
    ...
    switch (atoi(caseValue)) {
        ...
        case 35: {
            /* v2.0.7: QC Tool's QC-024 "Verify AGENTIX version" publishes
             * {"burnUID":...} to /rx/<sn>/35 and expects firmwareVersion in
             * the response.  Pre-v2.0.7 this used to return the on-disk
             * config.json contents, which the QC tool can't use.  We now
             * GET /netsdk/system/deviceinfo and publish that — the
             * response carries firmwareVersion, deviceName, serialNumber,
             * etc. */
            printf("Handling case 35 (firmware version GET)\n");
            send_https_request("/netsdk/system/deviceinfo", "GET", "", response, sizeof(response));
            response[sizeof(response) - 1] = '\0';
            sanitize_response(response);
            publish_mqtt_message(client, response, atoi(caseValue));
        ...
        case 81: {
            /* v2.0.7: Restored to tampering-detect GET.
             *
             *   v1.13.x used case 81 for provider_srt.conf push (the file-
             *   based config delivery).  v2.0.x then hijacked it for the
             *   brand-neutral retained payload (in-memory, via
             *   apply_ap2p_conf_payload).  But the ArcisAI Kitty QC Tool
             *   (CameraUtilityTool/qc_steps.py:_qc_040) expects case 81 to
             *   be Tampering Detect GET (mirror of case 66 but kept on the
             *   well-known number).  Conflict → all tampering QC steps
             *   timed out on v2.0.x.
             *
             *   Fix: retained config moved to case 90 (mqtt_thread.c).
             *   This case once again handles tampering GET. */
            printf("Handling case 81 (tampering detect GET)\n");
            send_https_request("/NetAPI/SmartDetect/Tamper", "GET", "", response, sizeof(response));
            response[sizeof(response) - 1] = '\0';
            sanitize_response(response);
            publish_mqtt_message(client, response, atoi(caseValue));
        ...
        case 83: {
            /* v2.0.7: Network configuration GET — new in v2.0.7.
             *
             *   ArcisAI Kitty QC Tool's QC-022 publishes {"deviceId":...}
             *   to /rx/<sn>/83 and expects a response with ip/gateway/dns
             *   fields.  Augentix `Update` sheet item 2 says GET endpoint
             *   is /NetAPI/R.Sync.Stat.NetWork.  Prior to v2.0.7 this case
             *   didn't exist in legacy_cases.c at all (QC tool always
             *   timed out). */
            printf("Handling case 83 (network config GET)\n");
            send_https_request("/NetAPI/R.Sync.Stat.NetWork", "GET", "", response, sizeof(response));
            response[sizeof(response) - 1] = '\0';
            sanitize_response(response);
            publish_mqtt_message(client, response, atoi(caseValue));
            break;
        ...
        case 90: {
            /* v2.0.7: brand-neutral retained-config push — moved here from
             * case 81 to free /81 for tampering (see above).  Actual handling
             * is in mqtt_thread.c:message_arrived → apply_ap2p_conf_payload().
             * This fallthrough is the safety net for when the case-90 message
             * leaks past the v2-router somehow (it shouldn't). */
            break;
        }
        default:
...
int reconnect_mqtt(void) {
    ...
    for (;;) {  // keep trying until connected, then run loop until drop
        for (int i = 0; i < uri_count; ++i) {
            ...
            MQTTClient_setCallbacks(client, client, onConnectionLost, legacy_message_arrived, NULL);
...
/* v2.0.7: Extended first-init sequence — fires once per process from
 * mqtt_thread.c after subscribe (see ap2p_legacy_startup_thread below).
 *
 * On every camera boot, push:
 *   - ini contents (case 1)
 *   - channelName (cameraname_Change)
 *   - video input config       (case 6)
 *   - image config             (case 38)
 *   - encoder ch102 config     (case 39)
 *   - encoder ch101 config     (case 2)
 *   - deviceInfo               (case 8)   ← new in v2.0.7
 *   - NTP server set           (case 23)  ← was defined but never called
 *   - timezone set to IST      (case 59 helper) ← was missing entirely
 *
 * Sleeps reduced from 5 s to 1 s between publishes — the camera's
 * NetSDK GET endpoints return in <100 ms; the 5 s pauses added 20 s to
 * first-init for no reason.  Total v2.0.7 startup pulse: ~9 s.
 */

/* Helper — set timezone to IST (GMT+05:30).  Wraps case 59's PUT. */
void handle_case_59_set_ist(void) {
    printf("FUNCTION:- %s\n\n", __func__);
    send_https_request("/NetSDK/system/time/timeZone", "PUT",
                       "\"GMT+05:30\"", response, sizeof(response));
    response[sizeof(response) - 1] = '\0';
    sanitize_response(response);
    publish_mqtt_message(client, response, 59);
}

/* Helper — publish deviceInfo (mirror of case 8). */
void handle_case_8(void) {
    printf("FUNCTION:- %s\n\n", __func__);
    send_https_request("/netsdk/system/deviceinfo", "GET", "",
                       response, sizeof(response));
    response[sizeof(response) - 1] = '\0';
    sanitize_response(response);
    publish_mqtt_message(client, response, 8);
}

void run_startup_cases(void) {
    printf("== v2.0.7 startup pulse begins ==\n");
    get_ini_files();
    cameraname_Change();
    handle_case_6();    sleep(1);   /* video input             */
    handle_case_38();   sleep(1);   /* image                   */
    handle_case_39();   sleep(1);   /* encoder ch102           */
    handle_case_2();    sleep(1);   /* encoder ch101           */
    handle_case_8();    sleep(1);   /* device info             */
    handle_case_23();   sleep(1);   /* NTP set pool.ntp.org    */
    handle_case_59_set_ist();       /* timezone set IST        */
    printf("== v2.0.7 startup pulse complete ==\n");
}

/* Thread wrapper — invoked from mqtt_thread.c via pthread_create after
 * subscribe + brief settle window for the retained case-90 payload to
 * land.  Detached; runs once per process. */
void *ap2p_legacy_startup_thread(void *arg) {
    (void)arg;
    /* Give the retained config a moment to arrive + be applied. */
    sleep(3);
    run_startup_cases();
    return NULL;
}
...
/* ---------------------------------------------------------------------------
 * KEEPALIVE thread — v1.13.x parity restoration (v2.0.7-rc3).
 *
 * v1 kitty's `run_main_loop()` (still defined in this file as dead code, lines
 * ~2466-2472) was an infinite loop calling handle_case_34() + handle_case_0()
 * every ~65 seconds — that's what published BOTH
 *
 *   torque/tx/<sn>/34   (Vmukti DeviceInfo)
 *   torque/tx/<sn>/0    (Keep Alive! + temperature + SD stats)
 *
 * The v2 single-binary refactor (main.c spawning mqtt+srt threads only)
 * silently dropped this loop because main.c does NOT call run_main_loop().
 * Cases 34 and 0 simply never fire.  Symptoms on the operator side:
 *   • mosquitto_sub -t torque/tx/+/0 -W 40   →  Timed out (camera looks dead)
 *   • QC tool shows camera as "no recent activity" / off-subnet
 *   • IoT-fleet alerting can't distinguish real outage from a camera that
 *     just wasn't speaking the heartbeat
 *   • Vmukti DeviceInfo periodic publish (case 34) also missing → dashboards
 *     that consume /tx/<sn>/34 (firmware version, hardware refresh) go stale
 *
 * Round-trip MQTT (case 8 deviceInfo etc.) works fine — proving the broker
 * connection is healthy; the only thing missing is the unsolicited periodic
 * publishes.  See docs/ROADMAP.md for regression history.
 *
 * Cadence: exact v1 parity — 65-s cycle, case 34 fires first, then 60 s
 * later case 0, then 5 s, repeat.  Doing it this way preserves any IoT-side
 * alerting rule that was written against the v1 timing (e.g. "if /tx/+/0
 * silent for > 90 s, alert" works without retuning).
 *
 * Loops until ap2p_state->shutdown_flag is set so SIGTERM exits cleanly.
 *
 * Safety: handle_case_0 / handle_case_34 → publish_mqtt_message(client, …)
 * which short-circuits when `client == NULL`.  So if mqtt_thread has
 * disconnected (sets g_client→NULL in legacy_init), this thread keeps
 * looping harmlessly until the next reconnect.
 * ------------------------------------------------------------------------- */

/* Cancellation-aware sleep — checks shutdown_flag every second so SIGTERM
 * doesn't wait up to a full minute before the thread exits. */
static int keepalive_sleep_or_exit(ap2p_state_t *state, int seconds) {
    for (int i = 0; i < seconds; i++) {
        if (state && state->shutdown_flag) return 1;
        sleep(1);
    }
    return 0;
}
...
void *ap2p_legacy_keepalive_thread(void *arg) {
    ap2p_state_t *state = (ap2p_state_t *)arg;

    /* Don't race the startup pulse (cases 6/8/23/38/39/59) — those need
     * the broker quiet while they read+publish.  Start ~10 s after thread
     * creation so the startup_thread has finished its 3-s settle + cases. */
    if (keepalive_sleep_or_exit(state, 10)) return NULL;

    while (!state || !state->shutdown_flag) {
        /* Exact v1 cadence: case 34 → 60 s → case 0 → 5 s → repeat. */
        handle_case_34();
        if (keepalive_sleep_or_exit(state, 60)) return NULL;
        handle_case_0();
        if (keepalive_sleep_or_exit(state, 5))  return NULL;
    }
    return NULL;
}
...
#if 0  /* AP2P v2.0: main() lives in src/ap2p/main.c; legacy_cases.c is included as a TU */
int main() {
    char NK_SNnumber[20];
    struct sigaction sa;
...
#endif  /* AP2P v2.0: end of guarded main() */

/* ==========================================================================
 *  AP2P glue
 * ==========================================================================
 *
 *  Wires the legacy globals (client, MQTT_TOPIC_RECEIVE, MQTT_TOPIC_SEND,
 *  MQTT_HOST, USERNAME, PASSWORD) from the values that ap2p's mqtt_thread.c
 *  already discovered from config.json.  Called once after the broker
 *  connection is established; legacy case handlers then publish responses
 *  through the same paho client.
 */
#include "legacy_cases.h"

void ap2p_legacy_init(MQTTClient mqtt_client,
                      const char *node_id,
                      const char *mqtt_url,
                      const char *username,
                      const char *password)
{
    client = mqtt_client;
    if (node_id) {
        snprintf(MQTT_TOPIC_RECEIVE, sizeof(MQTT_TOPIC_RECEIVE),
                 "torque/rx/%s/", node_id);
        snprintf(MQTT_TOPIC_SEND, sizeof(MQTT_TOPIC_SEND),
                 "torque/tx/%s/", node_id);
    }
    if (mqtt_url)  { strncpy(MQTT_HOST, mqtt_url,  sizeof(MQTT_HOST)  - 1); MQTT_HOST[sizeof(MQTT_HOST)-1]='\0'; }
    if (username)  { strncpy(USERNAME,  username,  sizeof(USERNAME)   - 1); USERNAME[sizeof(USERNAME)-1]='\0'; }
    if (password)  { strncpy(PASSWORD,  password,  sizeof(PASSWORD)   - 1); PASSWORD[sizeof(PASSWORD)-1]='\0'; }
}
